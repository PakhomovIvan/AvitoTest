...
2.  **Запустите API:** API находится в папке `server` и должен быть запущен на порту 3000. Инструкции по запуску API должны быть предоставлены отдельно (так как это существующая часть задания). Обычно это включает в себя переход в каталог `server`, установку зависимостей и запуск скрипта запуска (например, `node server.js`).
